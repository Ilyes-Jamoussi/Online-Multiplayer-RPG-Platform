...
export class SocketService {
    ...
    onEvent<T>(event: SessionEvents | GameStoreEvents | InGameEvents | ChatEvents | GameLogEvents | NotificationEvents): Observable<SocketResponse<T>> {
        return fromEvent<SocketResponse<T>>(this.socket, event);
    }

    onSuccessEvent<T>(event: SessionEvents | GameStoreEvents | InGameEvents | ChatEvents | GameLogEvents | NotificationEvents, next: (data: T) => void): void {
    ...
    onErrorEvent(event: SessionEvents | GameStoreEvents | InGameEvents | ChatEvents | GameLogEvents | NotificationEvents, next: (message: string) => void): void {
